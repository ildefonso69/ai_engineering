...
Desde la Sesión 15 hay **un único `docker-compose.yml` en la raíz** del repo que levanta los
cinco servicios; este subproyecto ya no tiene el suyo.

```bash
cd ..                 # raíz del repo
cp .env.example .env  # al menos OPENAI_API_KEY o ANTHROPIC_API_KEY, y AI_SERVICE_TOKEN
docker compose up --build
```

⚠️ **En el fichero estricto el servicio IA NO publica el 8000 al host** — es la propiedad de
seguridad de la S15. Para trabajar contra `http://localhost:8000` (Swagger en `/docs`) añade el
...
Recorrido completo y las cinco comprobaciones en [`../docs/deployment-local.md`](../docs/deployment-local.md).

Redis arranca como servicio vecino para el cache exact-match del wrapper. El `/health` responde
sin autenticación a propósito (lo usa el healthcheck de Docker); todo lo demás exige la cabecera
`X-Service-Token` cuando `AI_SERVICE_TOKEN` está configurado.
...
```bash
# Fuera del contenedor (desde ai-service/, con el .env cargado):
...
```bash
# Archivo completo:
docker compose exec -T vector-db psql -U estimator -d estimator \
  < ai-service/scripts/sql_s08/01_create_hnsw.sql
...
El tuning de Postgres para builds de índices vive en `docker-compose.yml` (servicio `vector-db`): `shm_size`, `shared_buffers`, `maintenance_work_mem`, `max_parallel_maintenance_workers`. Valores conservadores de desarrollo; en producción escalan con la RAM.
